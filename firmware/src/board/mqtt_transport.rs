...
use core::{cell::RefCell, future::{Future, poll_fn}, pin::pin};
use embassy_net::tcp::{TcpSocket, Error};
...
impl ErrorType for SharedSocket<'_, '_> { type Error = Error; }
...
impl WifiRng {
    pub fn new() -> Self {
        ...
        Self(esp_hal::rng::Rng::new(unsafe { esp_hal::peripherals::RNG::steal() }))
    }
...
impl rand_core::RngCore for WifiRng {
    ...
    fn next_u64(&mut self) -> u64 { u64::from_le_bytes(self.bytes()) }
